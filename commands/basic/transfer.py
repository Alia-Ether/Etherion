...
@antispam
async def transfer_cmd(message: types.Message, user: EtherionUser):
...
@antispam
async def limit_cmd(message: types.Message, user: EtherionUser):
    limit = get_limit_cmd(user.status)
...
@antispam
async def give_money(message: types.Message, user: EtherionUser):
    win, lose = EtherionConst.emj()
...
@antispam
async def give_bcoins(message: types.Message, user: EtherionUser):
    win, lose = EtherionConst.emj()
    ...
    try:
        ...
        await message.answer(f'{user.url}, чтобы выдать бкоины нужно ответить на сообщение пользователя {lose}')
        return
