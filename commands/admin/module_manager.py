...
@antispam_earning
async def load_modules_next(call: types.CallbackQuery, user: EtherionUser):
...
@antispam_earning
async def dell_mod(call: types.CallbackQuery, user: EtherionUser):
...
async def catalog_modules(message: types.Message):
    ...
    try:
        response = requests.get("https://raw.githubusercontent.com/Alia-Ether/Etherion-mod/refs/heads/main/modules.json")
...
@antispam_earning
async def catalog_type(call: types.CallbackQuery, user: EtherionUser):
    global MOD_TYPE
...
@antispam_earning
async def catalog_modules_next(call: types.CallbackQuery, user: EtherionUser):
...
@antispam_earning
async def load_mod(call: types.CallbackQuery, user: EtherionUser):
    name = call.data.split("_")[1].split("|")[0]
